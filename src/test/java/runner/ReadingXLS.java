...
public class ReadingXLS {
	...
	public JSONObject getTestData(String sheetName,String flag,int iterationNumber,String xlsFilePath) {
		...
		int index=1;
		while(!xls.getCellData(sheetName, 0, dataStartRomNumber).equals("")) { //iterate over the data till its blank
			int colIndex=0;
			if(index == iterationNumber) {
				JSONObject json = new JSONObject();
				while(!xls.getCellData(sheetName, colIndex , colNamesRowNumber).equals("")) { //iterate over column till it gets blank
